...
def load_master_data():
    try:
        ...
        # Lọc lỗi theo nhóm 'may' hoặc 'tp' nếu cần, hiện tại lấy hết hoặc theo logic cũ
        if 'nhom_loi' in df_config.columns:
            # Nếu TP Đầu vào dùng chung lỗi may, giữ nguyên. Nếu có nhóm riêng, sửa 'may' thành 'tp'
            target_groups = ['may', 'tp'] 
...
with st.expander("📝 Thông tin chi tiết (SP, HĐ, Nguồn gốc...)", expanded=not st.session_state.header_locked):
    ...
    # Nguồn gốc
    nguon_goc_list = st.multiselect("Nguồn gốc (Nơi may)", LIST_NOI_MAY, disabled=disable_hd)
...
if inspection_result == 'Pass':
    ...
    dept_prefix = NCR_DEPARTMENT_PREFIXES["TP_DAU_VAO"] # Mã bộ phận TP Đầu Vào
