...
def load_master_data():
    try:
        ...
        # Lọc lỗi theo nhóm 'may' nếu có cột nhom_loi, ngược lại lấy hết
...
with st.expander("📝 Thông tin chi tiết (SP, HĐ, Nguồn gốc...)", expanded=not st.session_state.header_locked):
    ...
    # 3 CỘT INPUT MỚI (NEW FIELDS - Chuẩn hóa theo FI)
    ...
    with col_new3:
        # Khách hàng auto từ Hợp đồng (sẽ hiển thị sau khi nhập HĐ)
        khach_hang_preview = ""
...
# --- TAB 1: ĐO ĐẠC & CHECKLIST (GIỐNG HỆT FI) ---
with tab_measure:
...
# --- TAB 2: CHI TIẾT LỖI (LOGIC CŨ CỦA MAY I) ---
with tab_defects:
    # Toggle Input Mode
...
if inspection_result == 'Pass':
    ...
    dept_prefix = NCR_DEPARTMENT_PREFIXES["MAY_I"] # Mã bộ phận May I
