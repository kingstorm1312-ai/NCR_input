...
from utils.ncr_helpers import get_all_users, update_user_status, update_user_info
...
if user_info.get("role") != "admin":
    st.error("⛔ Quyền truy cập bị từ chối! Chỉ dành cho Admin.")
    st.stop()
                    ...
                    with st.container(border=True):
                        ...
                        with c3:
                            ...
                            if col_b1.button("✅", key=f"app_{row['username']}", help="Duyệt (Active)"):
                                success, msg = update_user_status(row['username'], 'active')
                                if success:
                            ...
                            if col_b2.button("❌", key=f"rej_{row['username']}", help="Từ chối (Reject)"):
                                success, msg = update_user_status(row['username'], 'rejected')
                                if success:
                ...
                with st.form(key=f"edit_{row['username']}"):
                     ...
                     if st.form_submit_button("💾 Cập nhật thông tin"):
                         with st.spinner("Đang lưu..."):
                             success, msg = update_user_info(row['username'], new_role_key, new_dept_key)
                             if success:
                                 st.success(msg)
                                 st.cache_data.clear()
                                 st.rerun()
