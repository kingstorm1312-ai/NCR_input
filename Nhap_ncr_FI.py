...
def load_master_data():
    try:
        ...
        # Đảm bảo tên cột chuẩn (phòng khi sheet viết hoa/thường)
        # Mapping: noi_may, ten_loi, vi_tri_loi, muc_do
...
with c_vitri:
    vi_tri = st.selectbox("Vị trí", LIST_VI_TRI)
    ...
    if st.button("💾 LƯU DỮ LIỆU VÀO SHEET", type="primary", use_container_width=True):
        try:
            with st.spinner("Đang kết nối Google Sheets..."):
                ...
                # Lưu ý: Hàm này đọc toàn bộ sheet, nếu sheet quá lớn sau này cần tối ưu
                ...
                # st.rerun() # Refresh lại trang (cần cân nhắc nếu muốn giữ Header)
